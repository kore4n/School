...
std::vector<float> marching_squares(scalar_field_2d f, float isoval, float minx, float maxx, float miny, float maxy, float stepsize, float stepsizeX, float stepsizeY) {
...
int main(int argc, char* argv[])
{
	...
	std::vector<float> marchingVerts = marching_squares(f1, isoval, xmin, xmax, ymin, ymax, stepsize, stepsizeX, stepsizeY);
	...
	do {
		...
		glBegin(GL_LINES);
		// Draw vertical lines
		for (float x = xmin; x < xmax; x = x + stepsizeX) {
			glVertex2f(x, ymin);
			glVertex2f(x, ymax);
		}

		// Draw horizontal lines
		for (float y = ymin; y < ymax; y = y + stepsizeY) {
			glVertex2f(xmin, y);
			glVertex2f(xmax, y);
		}
		glEnd();
