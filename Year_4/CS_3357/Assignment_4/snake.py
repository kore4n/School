...
class SnakeGame:
    ...
    def get_state(self) :
        ...
        for uuid, p in self.players.items():
            # per player position is already in the form (a,b)*(a,b)*(a,b)*(a,b)
            current_player_info = str(p.get_color()) + "-" + str(p.get_pos())
            # print("color: " + str(p.get_color()))
            # print("position: " + str(p.get_pos()))
            all_players_info += current_player_info + "~"
            
        # remove the last tilda from all_players_info
        all_players_info = all_players_info[:-1]
        return all_players_info + "|" + snacks_pos
